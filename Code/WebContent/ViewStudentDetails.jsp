...
<br><br><br><br>
          <div class="carousel-item active" style="background-image:url(assets/img/stu6.jpg)">
  ...
  <main id="main">
    ...
    <div id="about" class="about-area area-padding">
      <div class="container">
       ...
       <br> <br> <br> <br> <br>
 <center><h2><strong> Student Register Details:</strong></h2></center>
<!-- <input type='text' id='txt_searchall' placeholder='Enter search text'>&nbsp; --> 
    <!--   <input type='text' id='txt_name' placeholder='Enter search name'> -->
      <br/><br>  <div class="fixTableHead">
    <center><table width='100%' border='1' style='border-collapse:collapse;'></center>

<!-- <table id="myTable"> -->
 
 <!--  <table class="table table-bordered" style="color:black border-spacing:0 5px;" > -->
  
  <%
                     Connection con=DBconnection.getConnection();
                     Statement st=con.createStatement();
                    /*  ResultSet rs=st.executeQuery("select farmer_reg.id,bankdetails.bankholdername from  farmer_reg inner join bankdetails on farmer_reg.email=bankdetails.email"); */
                    ResultSet rs=st.executeQuery("select addstuimage.id,stud_reg.rollno,stud_reg.name,stud_reg.gender,stud_reg.branch,stud_reg.year,stud_reg.prnno,stud_reg.email,stud_reg.contactno,stud_reg.address,addstuimage.status from addstuimage inner join stud_reg on addstuimage.rollno=stud_reg.rollno ");
                    //while(rs.next())
                     if(rs.next() == false) {
                    	%>
                    	<h6 ><b>No Users Present</b></h6>
                    	<%
                     }
                    else{
                    	int count=0;
                    	do{	 
                    	 ...
                    	 <tr class="header" style="color:blue" >
                    	<!--  <th>Student profile</th> -->
		                  ...
		                  <%}%>
                    	 <tr>
                    	<td><img alt="Profile Pic" class="img-circle" src="profiepicstu.jsp?id=<%=rs.getInt(1) %>"  width="100px" height="80px"/></td> 
                     ...
                     <%-- <td style="color:black"><%=rs.getString(13) %></td>   --%>
                    	...
                    	<th><a href="StatusStuImage?id=<%=rs.getInt(1)%>&status=<%=rs.getString(11)%>"><%=rs.getString(11)%></a></th> 
                    	<%--  <td><button  type="button" class="btn btn-danger"><b><a href="StatusServlet?id=<%=rs.getInt(1)%>&status=<%=rs.getString(9)%>"><%=rs.getString(9)%></a></b></button></td> --%>
                    	 
                    	<%--<th><a href="DeleteServlet?id=<%=rs.getInt(1)%>"><b>Delete</b></a></th> --%>
                    	 <td><button  type="button" class="btn btn-danger"><a href="DeleteStuServlet?id=<%=rs.getInt(1)%>"><b>Delete</b></a></button></td> 
                    	 </tr>                  	 
                    <%count++; }while(rs.next());}%> 
